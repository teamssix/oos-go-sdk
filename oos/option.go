...
func Connection(Connection string) Option {
	return setHeader(HTTPHeaderConnection, string(Connection))
}

// ACL is an option to set X-oos-Acl header
func ACL(acl ACLType) Option {
	return setHeader(HTTPHeaderoosACL, string(acl))
}

// Expiration is an option to set x-amz-expiration
func Expiration(expiration string) Option {
	return setHeader(HTTPHeaderoosExpiration, string(expiration))
}

// WebsiteRedirectLoca is an option to set x-amz-WebsiteRedirectLoca
func WebsiteRedirectLoca(websiteRedirectLoca string) Option {
	return setHeader(HTTPHeaderoosWebsiteRedirectLoca, string(websiteRedirectLoca))
}
...
func Meta(key, value string) Option {
	return setHeader(HTTPHeaderoosMetaPrefix+key, value)
}
...
// CopySource is an option to set X-oos-Copy-Source header
func CopySource(sourceBucket, sourceObject string) Option {
	return setHeader(HTTPHeaderoosCopySource, "/"+sourceBucket+"/"+sourceObject)
}

// CopySourceRange is an option to set X-oos-Copy-Source header
func CopySourceRange(startPosition, partSize int64) Option {
	...
	return setHeader(HTTPHeaderoosCopySourceRange, val)
}

// CopySourceIfMatch is an option to set X-oos-Copy-Source-If-Match header
func CopySourceIfMatch(value string) Option {
	return setHeader(HTTPHeaderoosCopySourceIfMatch, value)
}

// CopySourceIfNoneMatch is an option to set X-oos-Copy-Source-If-None-Match header
func CopySourceIfNoneMatch(value string) Option {
	return setHeader(HTTPHeaderoosCopySourceIfNoneMatch, value)
}

// CopySourceIfModifiedSince is an option to set X-oos-CopySource-If-Modified-Since header
func CopySourceIfModifiedSince(t time.Time) Option {
	return setHeader(HTTPHeaderoosCopySourceIfModifiedSince, t.Format(http.TimeFormat))
}

// CopySourceIfUnmodifiedSince is an option to set X-oos-Copy-Source-If-Unmodified-Since header
func CopySourceIfUnmodifiedSince(t time.Time) Option {
	return setHeader(HTTPHeaderoosCopySourceIfUnmodifiedSince, t.Format(http.TimeFormat))
}

// MetadataDirective is an option to set X-oos-Metadata-Directive header
func MetadataDirective(directive MetadataDirectiveType) Option {
	return setHeader(HTTPHeaderoosMetadataDirective, string(directive))
}

// ObjectACL is an option to set X-oos-Object-Acl header
func ObjectACL(acl ACLType) Option {
	return setHeader(HTTPHeaderoosObjectACL, string(acl))
}
...
func ObjectStorageClass(storageClass StorageClassType) Option {
	return setHeader(HTTPHeaderoosStorageClass, string(storageClass))
}

// RequestPayer is an option to set payer who pay for the request
func RequestPayer(payerType PayerType) Option {
	return setHeader(HTTPHeaderoosRequester, string(payerType))
}

// Set datalocation when an object is uploaded
func ObjectDataLocation(location string) Option {
	return setHeader(HTTPHeaderXctyunDataLocation, location)
}

// StorageClass bucket storage class
func StorageClass(value StorageClassType) Option {
	return setHeader(storageClass, string(value))
}

// put or get  object x-amz-limit
func Limit(limit string) Option {
	return setHeader(HTTPHeaderXamzLimit, limit)
}

func LimitForParam(limit string) Option {
	return addParam(HTTPHeaderXamzLimit, limit)
}
...
// Process is an option to set x-oos-process param
func Process(value string) Option {
	return addParam("x-oos-process", value)
}
