...
func (bucket Object) InitiateMultipartUpload(objectKey string, options ...Option) (InitiateMultipartUploadResult, error) {
...
// error    it's nil if the operation succeeds, otherwise it's an error object.
func (bucket Object) UploadPart(imur InitiateMultipartUploadResult, reader io.Reader,
...
// error    it's nil if the operation succeeds, otherwise it's an error object.
func (bucket Object) UploadPartFromFile(imur InitiateMultipartUploadResult, filePath string,
...
// error    it's nil if the operation succeeds, otherwise it's an error object.
func (bucket Object) DoUploadPart(request *UploadPartRequest, options []Option) (*UploadPartResult, error) {
...
// partSize    the part size
// partNumber    the part number, ranges from 1 to 10,000. If it exceeds the range oos returns InvalidArgument error.
...
//
//	CopySourceIfNoneMatch, CopySourceIfModifiedSince  CopySourceIfUnmodifiedSince.s
//
// UploadPart    the return value consists of PartNumber and ETag.
// error    it's nil if the operation succeeds, otherwise it's an error object.
func (bucket Object) UploadPartCopy(imur InitiateMultipartUploadResult, srcBucketName, srcObjectKey string,
	startPosition, partSize int64, partNumber int, options ...Option) (UploadPart, error) {
	var out UploadPartCopyResult
	var part UploadPart

	var opts = make([]Option, 0, 0)
...
// error    it's nil if the operation succeeds, otherwise it's an error object.
func (bucket Object) CompleteMultipartUpload(imur InitiateMultipartUploadResult,
...
//
// error    it's nil if the operation succeeds, otherwise it's an error object.
func (bucket Object) AbortMultipartUpload(imur InitiateMultipartUploadResult, options ...Option) error {
	params := map[string]interface{}{}
	params["uploadId"] = imur.UploadID
...
// error    it's nil if the operation succeeds, otherwise it's an error object.
func (bucket Object) ListUploadedParts(imur InitiateMultipartUploadResult, options ...Option) (ListUploadedPartsResult, error) {
...
// error    it's nil if the operation succeeds, otherwise it's an error object.
func (bucket Object) ListMultipartUploads(options ...Option) (ListMultipartUploadResult, error) {
